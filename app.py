...
def train_all_models(X_train, X_test, y_train, y_test):
    ...
    for idx, (name, model) in enumerate(models.items()):
        ...
        results.append({
            ...
            'test_mape': round(np.mean(np.abs((y_test - y_pred_test) / (y_test + 1))) * 100, 2)
        })
...
def main():
    ...
    page = st.sidebar.radio(
        "Navigation",
        ["About", "Predict Demand", "Model Comparison", "Data Summary"]
    )
    ...
    X_test = scaler.transform(X[~train_idx])

    results_df, trained_models = train_all_models(X_train, X_test, y[train_idx], y[~train_idx])
    ...
    if page == "About":
        ...
        show_model_comparison_page(results_df, trained_models, y[~train_idx])
    ...
    with col1:
        ...
        if submit:
            ...
            if prediction > stock_level:
                st.warning(f" Predicted demand ({prediction:.0f}) exceeds current stock ({stock_level}). Consider reordering.")
            else:
                st.info(f" Stock level is sufficient for predicted demand.")
...
def show_model_comparison_page(results_df, trained_models, y_test):
    ...
    <div class="best-model-card">
        ...
        <div style="display: flex; justify-content: space-around; margin-top: 1.5rem;">
            ...
            <div>
                <p style="color: #660; margin-bottom: 0.5rem;">R² Score</p>
